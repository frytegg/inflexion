...
| **▶ NEXT**       | **P3.5 — Routing** — `createSwap` routes the LP to the **cheaper of {pool price, best MM quote}**; record the path + load actually charged on `SwapRecord`. Then **P3.8** (I10 fuzz on both paths + Path-A mainnet-fork test + re-confirm I1–I9), **P3.9** (Slither on new contracts), **P3.10** (Sepolia deploy of the full set), **P3.11** (commit milestone). **P3.1–P3.4 + P3.6 + P3.7 DONE** (`521e77c`/`f69e200`/`e639dda`/`8fa6a84`; 137 contract tests green). **Two LATENT items on the personal PC (WSL2/Nitro):** P2.2 part-2 (FairValueOracle Stylus benchmark) and the Rust-vs-Solidity decision that gates **P2.3/P2.4/P2.5**. _(Task 5.14 Sepolia deploy folded into P3.10.)_                                                                                                                                                                                                                                                                                                                                                       |
...
| **Last commit**  | `feat(core): P3.4 Path-B premium derives from on-chain FairPremium (loadBps)` (`8fa6a84`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| **Last update**  | 2026-06-02 — **P2 (Solidity) + P3 core landed.** Committed in order: P2.2 part-1 FairValueOracle (`7fea916`), P3.1+P3.1b dual-tranche ConvexityVault (`521e77c`), P3.3/P3.6/P3.7 Path-A createSwap + I10 load stack + settle dispatch (`e639dda`), P3.2 EIP-1271 vault-signer (`f69e200`), **P3.4 Path-B `loadBps` schema (`8fa6a84`)** — both paths now price off the on-chain FairPremium; I10 enforced on both (Path A by clamp, Path B by `loadBps ≤ maxLoadBps`). **137 contract tests green; settle/MaxIL/I1–I9 untouched.** Spec v4.0 already mandates `loadBps` (§4.3) so no spec edit was needed. **LATENT (personal PC, WSL2/Nitro):** FairValueOracle Stylus benchmark (P2.2 pt-2) + the Rust-vs-Solidity decision gating P2.3/2.4/2.5. **Still owed at P5: fold tranching-in-launch into spec §8/§14/§18.** `params.json`/`params.py` unchanged.                                                                                                                                                                                            |
