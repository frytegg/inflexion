...
| **▶ NEXT**       | **P2.1 — `VolOracle.sol` (σ-EWMA)** then **P2.2 — `FairValueOracle` (exact Φ-sum)**. P1 (cvAMM single-asset quant) is **COMPLETE** — all 10 deliverables in `inflexion_quant.cvamm`/`prices`/`depositor`/`roadmap`/`heavy`; final numbers in `quant/params.cvamm.schema.json`; the verify-first closed-form gate is green (≡ il.py to ~5e-11), so P2 can wire the on-chain pricer. _(Task 5.14 Sepolia deploy stays paused → P3.x.)_   |
...
| **Quant track**  | **P1 COMPLETE (158 tests green).** cvAMM single-asset engine: `cvamm.py` (exact closed-form fairRate + CVaR/baseLoad + two skews + I10 clamp), `prices.sigma_ref`, `depositor.py`, `roadmap.py`, `heavy.py` (real-measure run). PARTIAL stack → `inflexion_quant.legacy` subpackage (+ `tests/legacy`) with compat shims; `params.json`/`params.py` **byte-unchanged** (frozen). Final cvAMM numbers in `params.cvamm.schema.json`; run output in `cvamm_heavy_results.json`; verdict in `HEAVY_CALIBRATION.md`. **KEY:** unhedged single-pair pool is INFEASIBLE for the SPEC safety targets at a marketable load (binding: 1-in-100 & P(losing month); intrinsic to single-asset vol-selling); no-bad-debt verified path-by-path; safe product = SENIOR tranche + pool-hedge (roadmap). Adversarial 6-agent audit: GO. |
| **Last update**  | 2026-06-02 — **P1 complete (P1.1–P1.14).** cvAMM single-asset calibration: exact closed-form fairRate (≡ il.py ~5e-11; reproduces MaxIL 1.27/2.56/5.23/13.76% + the surface + Example A $1,280/$906/$1,042), σ_ref EWMA, baseLoad from CVaR-collapse (lone-writer ~100% → diversified 78.5% @ρ=0, 100% @ρ=1 synchronized), two skews, I10 maxLoadBps (premium≤MaxIL), depositor disclosure + roadmap levers. Heavy real-measure run (fat-tail + regime-clustering + crash-correlation, episode-anchored — CoinGecko 401, fallback labelled): no-bad-debt on every path; unhedged INFEASIBLE → structural levers (util cap + pool-hedge + senior tranche). 6-agent adversarial audit: GO. Two commits: machinery `918f1b4` + final params. |
