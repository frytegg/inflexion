...
|                  |                                                                                                                                                                                                                                                                                                                                                                           |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Phase**        | 5 — `InflexionCore.sol` (architecture + invariant suite + Solidity `ILMath` complete; Sepolia deploy + Slither + gas pass deferred). **Phase 2 Stylus `ILMath` complete on WSL2** — local-Nitro deploy + bench (2.10–2.12) deferred                                                                                                                                       |
| **▶ NEXT**       | Pick one: (a) **Task 5.11** mainnet-fork integration test (real Arbitrum WETH/USDC v3 NFT, full create→settle cycle), (b) **Task 5.12** gas pass + Slither (5.13), or (c) **Task 5.14** Sepolia deploy. _Phase 2 (Stylus `ILMath`) is now complete on WSL2 — only 2.10–2.12 (local-Nitro deploy + Solidity integration + gas bench) remain._                              |
...
| **Last commit**  | `feat(stylus): ILMath with computeMaxIL + computeIL + full test suite + MATH.md` on branch `phase-2-ilmath-stylus` (PR pending). _Prior main HEAD: Solidity ILMath + TickMath port + InflexionCore tick-derived sqrt prices (97/97 suite)._                                                                                                                               |
...
| **Last update**  | 2026-05-31 — Stylus `ILMath` (Rust) landed on WSL2: pure-compute port of `ILMath.sol`, 29/29 host tests green; `cargo stylus check` passes (12.7 KB, activates) + `export-abi` clean. Tasks 2.2–2.9 + 2.13 done                                                                                                                                                           |
| **Blockers**     | Phase 2 Stylus **unblocked & landed** on the WSL2 (home) machine — 2.2–2.9 + 2.13 complete, `check`/`export-abi` green. Remaining 2.10–2.12 need a live local Nitro dev node (Group C). Windows MSVC native is still broken (`native_keccak256` link error in `stylus-proc`, no upstream fix), so Stylus dev must stay on WSL2 (see `RUNBOOK.md` → "Stylus development"). |
...
- [ ] **2.10 — Deploy `ILMath` to local Nitro** — `cargo stylus deploy --endpoint http://localhost:8545 --private-key $DEPLOYER_PRIVATE_KEY`. Record address. _(Deferred — needs a live local Nitro dev node. Toolchain validated on WSL2: `cargo stylus check` passes against the public Arbitrum Sepolia RPC — contract size **12.7 KB**, activation simulated (data fee ≈ 0.000097 ETH) — and `cargo stylus export-abi` emits a clean `IILMath` interface.)_
- [ ] **2.11 — Solidity integration test** — Foundry test calling `IILMath(addr).computeMaxIL(...)` and comparing to a Solidity reference implementation within rounding tolerance. _(Deferred with 2.10 — needs a deployed address.)_
- [ ] **2.12 — Gas benchmark** — measure `computeMaxIL` Stylus vs Solidity reference; record ratio in `docs/MATH.md`. (Spec claims ~10×; verify or adjust the pitch claim.) _(Deferred with 2.10 — needs an on-chain deployment.)_
