...
> ## ✅ REDEPLOY COMPLETED — 2026-06-05 (`e88e7bc`)
>
> The full-fresh redeploy to Arbitrum Sepolia is **DONE** — this checklist is now a
> **historical record**, not a to-do. Audited correct + consistent (registry /
> subgraph / SDK; 171 forge tests). **Do NOT re-run the redeploy.**
>
> **What shipped:** fresh Stylus FVO `0x98a6aa75…` (init'd to the FRESH VolOracle —
> NOT the old `0x10E3…`, see below) + core `0xC19865cF…` @ `optimizer_runs=1500`
> (23,934 B) + dual-tranche ConvexityVault `0xDE2fFeBA…` + 9 markets + fee-500 pool +
> ~$100k LP + funded tranches. Deploy block **274081134** = subgraph startBlock.
> Steps 1–5 + 8 below are DONE; the event-order fix (`f6f3872`) and marketId-parity
> tests are merged.
>
> **What remains (post-redeploy — see ROADMAP ▶ NEXT):** (a) the on-chain
> **Path-A/Path-B lifecycle demo** (smoke `swapId 1` + `createSwapPathA` +
> `createSwapRouted` + `settle` — the moat events have never fired live) and the
> **manual write-back of the new swapIds/figures into the registry**; (b) **subgraph
> deploy** (step 6, home-PC `build:wasm`/`deploy`) + **set `SUBGRAPH_URL`** (step 7);
> (c) host the API + engine. Section detail below is kept for the record; where it
> still reads pre-redeploy tense, the banner above governs.
...
- **keccak `marketId` cross-check passes bit-for-bit** — `deriveMarketId`'s tight-packed preimage reproduces the LIVE on-chain marketIds three independent ways (viem `encodePacked`, a manual replica of `deriveMarketId`, and the registry value). Locked by `packages/contracts/test/MarketIdParity.t.sol` + `packages/sdk/src/marketid.parity.test.ts`, both pinned to the **fresh-redeploy** ids: `marketId_fee500_7d` `0x67c4bee1…9d69`, `shortMarketId_fee500_300s` `0xb8bbd684…39c2`. _(The earlier verification used the prior run's ids `0xd1aa1fad…`/`0xacbeed…`; the full-fresh redeploy deployed FRESH demo tokens, so the ids changed — the parity tests + registry now carry the live values.)_

**⚠️ Found + fixed during this verification — commit `f6f3872` (MERGED to `main` BEFORE the redeploy `e88e7bc` ✓):** the WASM build is green, but a runtime event-order bug it cannot catch surfaced. The contract emits `SwapCreated → QuoteFilled → SwapPriced`, but `handleQuoteFilled` read `poolLoadWad`/`cappedAtMaxIL` — written only by the LATER `handleSwapPriced` — so Signal-2 `sumSpreadWad` was never accumulated and capped Path-B fills corrupted `sumMMLoadBps` (breaking the NON-CIRCULAR invariant). The spread + non-circular MM-load accumulation was relocated into `handleSwapPriced`. Also hardened: `NPM_ADDRESS` templated from the deployment registry (`src/generated-addresses.ts`, regenerated by `prepare:manifest`) instead of hardcoded; the two marketId parity tests above. **Merged before the redeploy ✓** — the moat dataset begins at the redeploy block and these aggregates are unreconstructable retroactively, so the fix had to land first (it did, in `e88e7bc`'s ancestry).
...
2. ✅ DONE — `forge script script/Deploy.s.sol` redeployed the Solidity stack (InflexionCore + the `CvammPricing` lib with `loadComponents`). **Correction to the original plan:** a **FRESH** Stylus FairValueOracle (`0x98a6aa75…`) was deployed — NOT the old `0x10E3…`. The old FVO was set-once-`init`'d to the OLD VolOracle, so reusing it would have orphaned the fresh vol's σ_ref; `setCvamm` wires the new core to the fresh FVO + fresh VolOracle (registry `deployment._note`).
